 ...
 * 
 * import io.github.anantharajuc.sbmwa.exception.CustomException;
 * 
 * // We should use OncePerRequestFilter since we are doing a database call,
 * there is no point in doing this more than once public class JwtTokenFilter
 ...
 * filterChain) throws ServletException, IOException { String token =
 * jwtTokenProvider.resolveToken(httpServletRequest); try { if (token != null &&
 * jwtTokenProvider.validateToken(token)) { Authentication auth =
 * jwtTokenProvider.getAuthentication(token);
 * SecurityContextHolder.getContext().setAuthentication(auth); } } catch
 * (CustomException ex) { //this is very important, since it guarantees the user
