...
public class StudentAttendanceServiceImpl	 implements IStudentAttendanceService
{
	...
	@Override
	public StudentAttendanceEntity insertAttandanceDetails(StudentAttendanceEntity attendanceEntity) 
	{
		log.info("-----> saveAttendanceDetails serviceImpl");
		  
		log.info("-----> attendanceEntity "+ attendanceEntity.getAttendancestatus());
		StudentAttendanceEntity attEntity = new StudentAttendanceEntity ();
	...
	private void getStudentData(StudentAttendanceEntity attEntity, Long studentid) {
	...
	@Override
	public StudentAttendanceEntity updateAttandanceDetails(Long id, StudentAttendanceEntity attenEntityUpdated) {
		log.info("-----> updateAttandanceDetails serviceImpl");
		StudentAttendanceEntity attEntity = attendanceEntityRepository.findById(id)
				.orElseThrow(() -> new ResourceNotFoundException("StudentAttendanceEntity", "id", id));
		if(attEntity!= null) {
	...
	public void deleteAttandance(Long id) {
		...
		StudentAttendanceEntity attendanceEntity = attendanceEntityRepository.getById(id);
		if(attendanceEntity!=  null) {
			attendanceEntityRepository.deleteById(id);
		}
	}
	...
	@Override
	public StudentAttendanceEntity updateAttandanceById(Long studId) {
		log.info("-----> updateAttandanceById serviceImpl");
		long millis=System.currentTimeMillis();  
		java.sql.Date date=new java.sql.Date(millis);  
		System.out.println(date);
		StudentAttendanceEntity attEntity = attendanceEntityRepository.getStudentById(studId, date);
	...
	private void calculateDuration(StudentAttendanceEntity attEntity) {
		Date d1 = null;
		Date d2 = null;
